    ...
    with torch.no_grad():
        ...
        while len(new_indexes) < num_of_data_to_add and len(unlabeled_loader.dataset) > 0:
            for data_unlabeled in unlabeled_loader:
                ...
                _, to_return = torch.topk(diff, 2, largest=False)
                for index in torch.gather(indexes, 0, to_return).tolist():
                    if len(new_indexes) < num_of_data_to_add:
                        new_indexes.append(index)
                        to_delete.append(index)
    ...
    with torch.no_grad():
        ...
        while len(new_indexes) < num_of_data_to_add and len(unlabeled_loader.dataset) > 0:
            for data_unlabeled in unlabeled_loader:
                ...
                _, to_return = torch.topk(entropy, 2)
                for index in torch.gather(indexes, 0, to_return).tolist():
                    if len(new_indexes) < num_of_data_to_add:
                        new_indexes.append(index)
                        to_delete.append(index)
...
def main(seed_size=None, train_batch_size=None, test_batch_size=None, num_of_data_to_add=None, sampling_parameter=None):
    directory = './data'
    ...
    unlabeled_loader = data.DataLoader(unlabeled_dataset, batch_size=train_batch_size, shuffle=True,
                                       num_workers=multiprocessing.cpu_count())
    test_loader = data.DataLoader(dataset=test_set, batch_size=test_batch_size, pin_memory=True)
